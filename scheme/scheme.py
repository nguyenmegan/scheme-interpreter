...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    try:
        ...
        raise SchemeError('incorrect function')
    # END Question 4
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    fns = expressions
    while fns is not nil:
        value = scheme_eval(fns.first, env)
        if scheme_true(value) == False:
            return False
        fns = fns.second
    return value
    # END Question 14B
...
def check_formals(formals):
    ...
    listofsymbols = []
    while formals is not nil:
        if scheme_symbolp(formals.first) == False:
            raise SchemeError("not a valid symbol")
        elif formals.first in listofsymbols:
            raise SchemeError("repeated symbol")
        listofsymbols.append(formals.first)
...
def do_mu_form(expressions, env):
    ...
    #     raise SchemeError("lacks list of formals")
